...
function getSorted(arr){{var key=document.getElementById('sortSel').value;var copy=arr.slice();if(key==='adv-desc'){{copy.sort(function(a,b){{return(hasAdv(b)?1:0)-(hasAdv(a)?1:0)||b.badges.length-a.badges.length;}});}}else if(key==='badge-desc'){{copy.sort(function(a,b){{return b.badges.length-a.badges.length;}});}}else if(key==='name'){{copy.sort(function(a,b){{return a.name.localeCompare(b.name,'ja');}});}}else{{copy.sort(function(a,b){{var od=getDeptOrder(a.dept)-getDeptOrder(b.dept);if(od!==0)return od;return a.dept.localeCompare(b.dept,'ja')||a.name.localeCompare(b.name,'ja');}});}}return copy;}}
function render(){{
  var f=getFiltered();
